 ...
 * — movement, editing, selection and history — all bound to one
 * buffer. The interpreter installs them; the Lisp in `../lisp/` calls
 * them.
 *
...
/**
 * Build the buffer primitives for one buffer.
 *
 ...
 */
export function createBufferPrimitives(buffer) {
  ...
  return {
    ...
    // --- selection ------------------------------------------------------
    'set-mark!': (args) => {
      buffer.setMark(args.length > 0 ? offset(args[0]) : buffer.point);
      return NIL;
    },
    'clear-mark!': () => {
      buffer.clearMark();
