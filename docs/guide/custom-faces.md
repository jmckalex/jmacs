...
value, not the literal default, so re-evaluating a `defcustom` (a hot
reload, `C-x C-r`) refreshes the metadata while a value the user has
customised survives. Like `define`, the form returns the variable's
name.
...
downstream. The widget honesty: `:list` gets a text field that cannot
round-trip a list, so treat list-valued settings (`*latex-command*`,
`*snippet-directories*`) as code territory — declare them so they are
documented and discoverable, change them from Lisp.
...
root is `jmacs` (registered with parent `nil`), and the stdlib hangs
`appearance`, `faces`, `latex`, `snippets`, `panes`, and others off
it. The customize buffer scoped to a group shows its documentation, a
link for each registered subgroup, and a row for each setting whose
`:group` names it — the tree is also the navigation. From Lisp,
...
`jmacs` and follows only registered links, so a setting filed under an
...
data directory: a comment header and one line per saved setting,
...
the face assigned to it. The attribute set is fixed: `:foreground` and
`:background` (hex colour strings), `:weight` (`:bold`), `:slant`
(`:italic`), `:underline` and `:strike-through` (booleans). An
attribute map is built with the `face` constructor, which takes
keyword pairs and tolerates any subset —
`(face :foreground "#e8a87c" :weight :bold)` evaluates to the
two-entry map `{:foreground "#e8a87c" :weight :bold}`. `defface`
registers a face with a docstring, an optional parent, and a default
per shipped theme:
...
(defface 'name
  ...
  :default-bright   (face …)
  :default-light    (face …)
  :default-midnight (face …))

(defface 'name from 'parent :doc "…" …)   ; inheriting form
```

Two syntactic notes. The face name is *quoted* — `defface` evaluates
the name position, unlike `defcommand` and `defcustom`, which take
bare names. And `from` is a literal word between the name and the
first keyword, naming a single parent face. Here is `comment` from
`themes.lisp`, the file that declares all fourteen token faces:
...
> is a *dark-background* theme with a saturated palette. A
> `:default-bright` block therefore wants light foregrounds on a dark
> surface, like `:default-dark` and `:default-midnight`; only
> `:default-light` sits on a light background.
...
global override, then the `defface` default for the active theme — and
an attribute still missing falls through to the parent face, resolved
the same way (the chain is walked to the top; a cycle is an error), so
a child face declares only what it changes. The override layers are
what cmd(customize-faces) and `set-face-attribute` write —
`(set-face-attribute 'comment :foreground "#ff5370")` globally, or
scoped with `:theme 'dark` or `:mode "LaTeX"`; the user-side story,
with `reset-face` and persistence to `faces.json`, is in
*Customization*.
...
```lisp
(define-theme 'dark
  "Mariana — the calm default dark scheme."
  (hash-map "--bg" "#2b333b" "--bg-editor" "#303841" …))
```
...
defaults. The active theme is just the `*theme*` setting — a `:choice`
over `dark`, `bright`, `light`, and `midnight` whose `:on-change`
calls the host's `apply-theme!` — so `(custom-apply! '*theme*
'midnight)` switches live: the chrome variables are rewritten and
every face re-resolves under the new theme, your overrides intact.

Can you author a fifth theme from Lisp? Honestly: halfway. A
`define-theme` in `init.lisp` registers its chrome, and
`(custom-apply! '*theme* 'mine)` adopts it — but `defface` has exactly
four per-theme slots, so every face falls back to its `:default-dark`
block, and the customize dropdown lists only the shipped four. New
palettes are better expressed as per-theme face overrides on an
existing theme.
...
(defgroup 'desk 'jmacs "Personal appearance switches for this machine.")
...
(defface 'loud-comment from 'comment
  ...
  :default-light    (face :foreground "#b07d3c" :weight :bold))
...
Evaluate the buffer (or restart) and try it at the REPL:
