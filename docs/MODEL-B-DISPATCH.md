...
- **`spine.js` / `server.js` / `app.js` / `main.js` are not in the suite.** `node --check` for JS syntax; a throwaway interpreter harness for embedded Lisp; then **live-verify**. `grep -a` on `spine.js` (it reads as binary).
